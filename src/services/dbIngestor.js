  ...
  if (typeof indexedDB === 'undefined') {
    try {
      ...
      const candidatePaths = [
        path.resolve('DATABASE DOWNLOADS/oracle-cards-20260725090335.jsonl'),
        path.resolve('database/oracle-cards-20260428090245.json'),
        path.resolve('public/data/cards.json')
      ];
            ...
            for await (const line of rl) {
              if (line.trim()) {
                try {
                  ...
                  cards.push(extractCardData(card));
                } catch (e) {}
              }
            }
          } else {
            const raw = JSON.parse(fs.readFileSync(dbPath, 'utf8'));
            cards.push(...raw.map(extractCardData));
          }
          if (cards.length > 0) {
            return cards;
          }
      ...
      console.warn('[dbIngestor] Error al leer base de datos local en Node:', e.message);
    }
  }
